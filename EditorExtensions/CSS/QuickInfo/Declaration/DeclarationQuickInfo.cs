    ...
    internal class DeclarationQuickInfo : IQuickInfoSource
    {
        ...
        private static UIElement CreateBrowserList(Dictionary<string, string> browsers)
        {
            ...
            foreach (string name in browserAbbr)
            {
                ...
                {
                    image.Opacity = 0.4;
                    image.Source = BitmapFrame.Create(new Uri("pack://application:,,,/WebEssentials2015;component/Resources/Images/Browsers/" + name + "_gray.png", UriKind.RelativeOrAbsolute));
                }
                else
                {
                    block.Text = browsers.ContainsKey("all") ? string.Empty : browsers[name];
                    image.Source = BitmapFrame.Create(new Uri("pack://application:,,,/WebEssentials2015;component/Resources/Images/Browsers/" + name + ".png", UriKind.RelativeOrAbsolute));
                }
